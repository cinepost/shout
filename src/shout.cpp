  ...
  try { 
    ...
    if (vm.count("input-file")) {
      ...
      for (vector<string>::const_iterator fi = files.begin(); fi != files.end(); ++fi) {
        //if (ribLoader.load(*fi, loadedScene)) {
      	if ( 1 == 1 ) {
          BOOST_LOG_TRIVIAL(info) << "File " << *fi << " loaded successfuly !";
          exit(EXIT_SUCCESS);
        }
      }
    } else {
      ...
      //if (ribLoader.load(std::cin, loadedScene)) {
      if ( 1 == 1) {
        BOOST_LOG_TRIVIAL(info) << "Scene loaded successfuly !";
        exit(EXIT_SUCCESS);
      }
    }
    exit(EXIT_FAILURE);
